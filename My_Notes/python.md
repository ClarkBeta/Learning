...
多进程模式的**优点**是稳定性高，子进程崩溃不影响主进程和其他子进程（如Apache早期采用此模式）；**缺点**是创建进程开销大（尤其在Windows下），且操作系统能同时运行的进程数有限，过多进程会导致调度问题。  

多线程模式**速度略快**于多进程，但**稳定性差**，因线程共享内存，单个线程崩溃可能导致整个进程崩溃（如Windows中程序报错强制退出）。  

**权衡与混合模式**：  
- Windows下多线程效率更高，故IIS默认采用多线程，但稳定性低于Apache。  
- 为平衡性能与稳定性，现代服务器（如IIS、Apache）转向**多进程+多线程混合模式**，但架构复杂度增加。  

**核心矛盾**：性能（多线程）与稳定性（多进程）的权衡，不同场景需差异化选择。
...
**1. 计算密集型任务**  
- **特点**：主要消耗 CPU 资源，如圆周率计算、视频解码等。  
- **多任务优化**：  
  - 任务数量应与 CPU 核心数相同，避免过多任务切换降低效率。  
- **语言选择**：  
  - **C 语言**等高效编译语言更合适，因其运行速度快。  
  - **Python 等脚本语言**效率低，不适合计算密集型任务。  

**2. IO 密集型任务**  
- **特点**：涉及网络、磁盘 IO，CPU 占用低，大部分时间在等待 IO（如 Web 应用）。  
- **多任务优化**：  
  - 可同时运行较多任务，提高 CPU 利用率，但需合理控制数量。  
- **语言选择**：  
  - **脚本语言（如 Python）**更合适，因其开发效率高，代码量少。  
  - **C 语言**对运行效率提升有限，因 CPU 占用本身很低。  

**核心结论**  
- **计算密集型** → **C 语言 + 任务数 = CPU 核心数**（减少切换开销）。  
- **IO 密集型** → **Python 等脚本语言 + 多任务**（提高并发，优化开发效率）。

异步IO

考虑到CPU和IO之间巨大的速度差异，一个任务在执行的过程中大部分时间都在等待IO操作，单进程单线程模型会导致别的任务无法并行执行，因此，我们才需要多进程模型或者多线程模型来支持多任务并发执行。

现代操作系统对IO操作已经做了巨大的改进，最大的特点就是支持异步IO。如果充分利用操作系统提供的异步IO支持，就可以用单进程单线程模型来执行多任务，这种全新的模型称为事件驱动模型。在多核CPU上，可以运行多个进程（数量与CPU核心数相同），充分利用多核CPU。由于系统总的进程数量十分有限，因此操作系统调度非常高效。用异步IO编程模型来实现多任务是一个主要的趋势。
### 11.5 分布式进程
```python
